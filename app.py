...
import json  # For storing profiles locally
...
PROFILE_STORE = "profiles.json"  # File to store user profiles
...
if st.session_state['page'] == 'form':
    ...
    with st.form(key="user_form"):
        ...
        if submitted:
            if not is_valid_name(name):
            ...
            elif email in profiles:  # Existing user
                ...
                profile = {
                    ...
                    "specific_needs": specific_needs_and_challenges,
                    "training": training,
                    "mode_of_training": mode_of_training,
                    "preferred_contact": prefered_time_contact_mode,
                }
                ...
                st.success("Profile created! Redirecting to chat...")
                st.session_state['page'] = 'chat'
                st.rerun()
...
# ----------------------
    ...
    if user_input:
        bot_response = chat_with_ai(user_input, scrape_website(WEBSITE_URL), extract_pdf_text(PDF_PATH), st.session_state['chat_history'])
